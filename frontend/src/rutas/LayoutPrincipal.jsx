...
export function LayoutPrincipal() {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Encabezado />
      <main className="flex-1 container-base py-6 pb-20 md:pb-6">
        <Outlet />
      </main>
      {/* Barra inferior móvil sticky */}
      <div className="md:hidden fixed bottom-0 inset-x-0 z-[70] bg-white border-t shadow h-16">
        <nav className="grid grid-cols-5 text-xs">
          ...
          <button type="button" onClick={() => { if (!isAuthenticated || !usuario?.id) { navigate('/login') } else { window.dispatchEvent(new Event('figumarket:openMessages')) } }} className="relative flex items-center justify-center py-3 text-gray-700" aria-label="Mensajes">
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-[18px] h-5 px-1 rounded-full bg-red-500 ring-2 ring-white text-white text-[10px] font-semibold flex items-center justify-center">{unreadCount}</span>
            )}
